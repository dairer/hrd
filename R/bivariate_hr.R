...
#' @param warmup (numeric) see ?rstan::sampling
#' @param thin (numeric) see ?rstan::sampling
#' @param init (numeric) see ?rstan::init
#'
#' @return (list(estimate (numeric), post (numeric), fitted_model (stan model))) List of 3 elements,
...
#'
#' @examples
#' x = rgev(100, loc = 1, scale = 1, shape = -0.1)
#' fit_gev_bay(x)
...
#
# # # # Transform margins to be GPD
# x = hrd::rgev(100, loc = 1.2,  scale = 1, shape = 0.2)
# # #
# # # # fit margins and copula jointly
# stan_data = list(N = 100,
#                  x = rgev(100, loc = 0.5, scale = 1, shape = 0.2),
#                  prior_mean_sig = 1,
#                  prior_sd_sig = 10,
#                  prior_mean_loc = 0,
#                  prior_sd_loc = 10,
...
#
# fitted_model
# rstan::traceplot(fitted_model)
#
#
#
# stan_data = list( N = 1000,
#                   y = rgev(1000, loc = 1, scale = 1, shape = 0.1))
#
#
# fitted_model = rstan::stan(
#   file = "test_stan.stan",
